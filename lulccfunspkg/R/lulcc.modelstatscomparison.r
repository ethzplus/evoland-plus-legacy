    ...
    if (length(table(model_eval_summary[, .SD, .SDcols = grouping_var])) == 2) {
      ...
      violin_plot <- ggwithinstats(
        ...
        ggplot.component = list(
          scale_y_continuous(limits = c(0.0, 1), breaks = seq(from = 0, to = 1, by = 0.1)),
          theme(
      ...
      Diff_plot_pred <- ggplot(grouped_dep_var) +
        ...
        labs(
          y = bquote(Delta ~ "Average " ~ .(str_replace(eval_metrics[i], "_", " ")) ~ " between specifcations"), # use bquote expression to include greek symbol Delta, object and string
      ...
      Diff_plot_size <- ggplot(grouped_dep_var) +
        ...
        labs(
          y = bquote(Delta ~ "Average " ~ .(str_replace(eval_metrics[i], "_", " ")) ~ " between training and test sets"), # use bquote expression to include greek symbol Delta, object and string
...
(because we are dealing with 'paired samples') one-way ANOVA so the Friedman test is appropriate")
        ...
        if (res$p < 0.05) {
          ...
          post_hoc <- frdAllPairsConoverTest(
          ...
          post_hoc_noutliers <- frdAllPairsConoverTest(
            y = model_eval_summary_noutliers[[eval_metrics[i]]], groups = model_eval_summary_noutliers[[grouping_var]],
      ...
      if (test_type == "parametric") {
        ...
        if (res$p < 0.05) {
          ...
          violin_with_pairwise <- violin_with_pairwise + geom_signif(
            inherit.aes = FALSE,
            data = ph_plot_res,
      ...
      model_performance_scatter <- ggplot(model_eval_summary) +
        ...
        labs(
          y = paste0("Average ", str_replace(eval_metrics[i], "_", " ")),
