#' Prepare transition tables for land use and land cover (LULC) simulation
#'
#' Create tables of transition rates for future time points for use in LULCC allocation
#' within Dinamica EGO. Read historical LULC data, create linearly extrapolated
#' transition rates, and modify them according to different scenarios. The process
#' involves several steps:
#'
#' @section Overall process:
#' 1. Prepare data and folders
#' 2. Create folder structure for scenario-specific transition rate tables
#' 3. Interpolate/extrapolate transition rates into future years
#' 4. Create transition tables for calibration periods
#' 5. Calculate expected LULC class areal change
#' 6. Modify transition rates for different scenarios
#' 7. Check the resulting transition areas against desired LULC class areas
#' 8. Calculate final modified transition rates
#'
#' @param config Configuration list containing paths and parameters. If not provided,
#'        it will be obtained from get_config()
#'
#' @return No return value. The function creates and saves various transition tables
#'         and auxiliary data for LULC simulation.
#'
#' @author Ben Black
#' @export
...
simulation_trans_tables_prep <- function(config = get_config()) {
  ...
  purrr::walk(
    scenario_names,
    function(x) ensure_dir(file.path(trans_rate_table_dir, x))
  )

  #### C- Interpolate/Extrapolate transition rates as reference for scenario modifications ####
  ...
  lulcc.transratesextrapolation <- function(
    ...
    Period_cols <- grep(
      paste(data_periods, collapse = "|"),
      colnames(trans_rate_table),
      value = TRUE
    )
    Final_period_col <- grep(
      data_periods[length(data_periods)],
      colnames(trans_rate_table),
      value = TRUE
    )
    ...
    names(trans_rate_table)[names(trans_rate_table) %in% Period_cols] <- c(
      sapply(
        data_periods,
    ...
    trans_rates_long <-
      trans_rate_table |>
      tidyr::pivot_longer(
        # `cols` was: initial year for each elem of data_periods
    ...
    Extrap_calibration_rates <- data.frame(matrix(
      nrow = length(unique(trans_rates_long$Trans_name)),
    ...
    for (Name in unique(trans_rates_long$Trans_name)) {
      ...
      Extrap_calibration_rates[
        Extrap_calibration_rates$Trans_name == Name,
        c(2:ncol(Extrap_calibration_rates))
      ] <- Pred

      # create scatterplot with fitted regression line
      Trans_plot <-
        ggplot2::ggplot(
          trans_rates_long[trans_rates_long$Trans_name == Name, ],
          ggplot2::aes(x = Year, y = Perc_rate)
        ) +
        ggplot2::geom_point() +
      ...
      # save plot
      ggplot2::ggsave(
        filename = paste0(Name, "_", single_multi_step, "_trans_rate_plot.jpg"),
        plot = Trans_plot,
        device = "jpeg",
        path = save_dir
      )
    ...
    Combined_table <- cbind(
      trans_rate_table,
      Extrap_calibration_rates[, c(paste(time_steps))]
    )
    ...
    openxlsx::write.xlsx(
      ...
      file.path(
        save_dir,
        paste0("extrapolated_trans_rates_", single_multi_step, ".xlsx")
      )
    )
  ...
  MS_extrap_trans_rates[MS_extrap_trans_rates < 0] <- NA_real_
  ...
  lulcc.savescenariotranstables <- function(
    ...
    sapply(Time_steps, function(year) {
      ...
      trans_rate_table <- trans_rate_table[
        trans_rate_table$Trans_name %in% Periodic_trans_names[[Time_period]],
      ]
      ...
      readr::write_csv(trans_rate_table, file = file_name)
  ...
  Calibration_time_steps <- seq(
    from = min(LULC_years),
    to = round(max(LULC_years) / step_length) * step_length,
    by = step_length
  )
  ...
  Historic_LULC_area <- read.csv(file.path(
    config[["trans_rates_raw_dir"]],
  ...
  Trans_areal_increase[, paste(Sim_time_steps)] <- NA_real_

  # create table to capture total area of lulc classes for future time points
  LULC_proj_area <- Historic_LULC_area[, c(
    "LULC_class",
    grep(Final_calib_year, names(Historic_LULC_area), value = TRUE)
  )]
  names(LULC_proj_area)[2] <- round(max(LULC_years) / step_length) * step_length
  ...
  for (i in 1:(length(Sim_years) - 1)) {
    calc_year <- as.character(Sim_years[i + 1])
    prev_year <- as.character(Sim_years[i])
  ...
  for (scenario in scenario_names) {
    ...
    Glacier_ncells <- sum(
      Glacier_index[[as.character(max(Sim_time_steps))]] == 1
    )
  ...
  Mod_LULC_areas[as.character(Sim_time_steps)] <- NA_real_
  ...
  Mod_area_tables <- lapply(
    names(Mod_area_tables),
    function(scenario) {
  ...
  Mod_trans_areas[paste(Sim_time_steps)] <- NA_real_
  ...
  Mod_trans_rates[as.character(Sim_time_steps)] <- NA_real_
  Mod_trans_rate_tables <- lapply(Scenario_area_mods$Scenario, \(x) Mod_trans_rates)
  ...
  for (Scenario in Scenario_area_mods$Scenario) {
    ...
    gross_cover_2060 <- unlist(
      Scenario_area_mods[
        Scenario_area_mods$Scenario == Scenario,
        colnames(Scenario_area_mods) != "Scenario"
      ]
    )
    ...
    Total_area <- sum(LULC_proj_area[, as.character(Sim_years[1])])
    ...
    for (x in seq_len(length(Sim_years) - 1)) {
      ...
      Mod_area_tables[[Scenario]][, paste(Sim_years[x + 1])] <-
        Mod_area_tables[[Scenario]][, paste(Sim_years[x])] + Step_change
    }
    ...
    for (calc_year in paste(Sim_time_steps)) {
      ...
      for (lulc_class in names(net_change)) {
        ...
        Perc_gain_losses[[Scenario]][[calc_year]][[lulc_class]][["Perc_gain"]] <-
          Perc_gain
        Perc_gain_losses[[Scenario]][[calc_year]][[lulc_class]][["Perc_loss"]] <-
          Perc_loss
        ...
        Gain_trans <- Trans_areal_increase[
          Trans_areal_increase$Final_class == lulc_class,
        ]
        ...
        if (nrow(Gain_trans) > 0 & Gain_area != 0) {
          ...
          for (trans in seq_len(nrow(Gain_trans))) {
            ...
            # calculate area of transition as proportion of required Gain_area according
            # to % contribution
        ...
        if (nrow(Loss_trans) > 0 && Loss_area != 0) {
          ...
          for (trans in seq_len(nrow(Loss_trans))) {
      ...
      Mod_trans_area_tables[[Scenario]][, calc_year] <- sapply(Final_trans_areas, function(A) {
        if (A >= 0) {
          A
        } else {
          0
  ...
  # first check of mismatches in areas, identifying synergistic transitions that
  # currently have a rate of zero (i.e they have been cancelled out in the 1st
  # approximation)
  # TODO check if this was meant to be called for side effects. the return value wasn't
  # used, so maybe it was just for interactive exploration?
  lulcc.projectedclassareasmismatch(
  ...
  # Loop function using multiple thresholds to see if focusing on the synergistic
  # transitions first helps
  Threshold_iterations <- list()
  num_reps <- 50

  for (t in seq(0.1, 5, 0.1)) {
    Iter_result <- lulcc.improvetransareadistrib(
      Scenario_class_area_tables = Mod_area_tables,
  ...
  # TODO this clearly isn't called for a side effect. remove interactive usage.
  sapply(Threshold_iterations, function(x) {
  ...
  Scenario_best_results <- data.table::rbindlist(lapply(
    unique(Improvement_results$Scenario),
    function(x) {
      ...
      return(Scenario_results[which.min(Scenario_results$min_mismatch_perc), ])
    }
  ))
  ...
  # TODO printed for interactive usage?
  print(sapply(Final_mismatch_check, function(x) {
    x[["Total_mismatch_perc"]]
  }))
  ...
  for (Scenario in names(Best_trans_area_tables)) {
    ...
    for (Sim_year in 1:(length(Sim_years) - 1)) {
      calc_year <- as.character(Sim_years[Sim_year + 1])
      prev_year <- as.character(Sim_years[Sim_year])

      # inner loop over transitions
      for (trans in seq_len(nrow(Best_trans_area_tables[[Scenario]]))) {
        ...
        Class_areas <- Proj_class_area_tables[[Scenario]][
          Proj_class_area_tables[[Scenario]][["LULC_class"]] == Initial_class,
        ]
  ...
  for (scenario in seq_along(Final_trans_rate_tables)) {
    ...
    for (i in paste(Sim_years)) {
      ...
      file_name <- file.path(
        ...
        Scenario_name,
        paste0(Scenario_name, "_trans_table_", i, ".csv")
      )

      # save
      readr::write_csv(trans_rate_table_subset, file = file_name)
    } # close loop over simulation years
