...
lulcc.evalfeatureselection2 <- function(
  ...
  Unique_covs <- lapply(Pre_selection_data, function(x) {
    ...
    cov_data <- x[, .SD, .SDcols = Predictor_table$pred_name]
  ...
  Long_results$Clean_cov_name <- str_remove_all(
    Predictor_table$pred_name[match(
      Long_results$covariate,
      Predictor_table$Layer_name
    )],
    paste(c("_mean_100m", "_100m", "_nhood"), collapse = "|")
  )
  Long_results$cov_group <- Predictor_table$pred_category[match(
    Long_results$covariate,
    Predictor_table$Layer_name
  )]
