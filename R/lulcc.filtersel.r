...
lulcc.filtersel <- function(
    transition_result,
    cov_data,
    categories,
    collin_weight_vector,
    embedded_weight_vector,
    focals = NULL,
    method, corcut = 0) {
  ...
  covdata.candidates.L1 <- lapply(
    covdata.candidates, function(y) {
      lulcc.covfilter(
        cov_data = y,
        method = method,
        trans_result = transition_result,
  ...
  # Level 2 for all selected currently removed because I don't need to filter beyond the
  # neighbourhood and suitability categories. covdata.candidates.L2 <-
  # LULCC.binarizedcovfilter(cov_data=do.call("cbind",covdata.candidates.L1),
  # method=method, trans_result= trans_result, weights=weights, corcut=corcut)

  # but this means that we do need to bind the seperate lists of covariates by
  # categories because this is ordinarily done within level 2
  covdata.candidates.bound <- do.call("cbind", covdata.candidates.L1)

  # return results
  names(covdata.candidates.bound) <-
    gsub("^.*\\.", "", names(covdata.candidates.bound)) # reformatting names to match original
  recompile_data <- list(
    transition_result = transition_result,
    covdata_collinearity_filtered = covdata.candidates.bound,
    collin_weight_vector = collin_weight_vector,
    embedded_weight_vector = embedded_weight_vector
  )
