...
lulcc.analysecovselection <- function(
  ...
  if (summary_level == "across_trans") {
    ...
    Dot_plot_num_cov_changes <- ggplot() +
      geom_point(data = long_df, aes(x = num_covs, y = Trans_redact, colour = FS_stage), alpha = 0.7) +
      ...
      scale_colour_discrete(
        type = ghibli_palettes$MononokeMedium,
    ...
    Agg_Dot_plot_num_cov_changes <- ggplot(long_df_agg) +
      ...
      geom_errorbar(
        data = long_df_agg[long_df_agg$FS_stage == "mean_embedded", ], aes(x = num_covs, y = Trans_redact, xmin = num_covs - sd_embedded, xmax = num_covs + sd_embedded),
        width = .3,
  ...
  if (summary_level == "Bioregion") {
    ...
    All_covariates_by_Bioregion <- lapply(Bioregion_Covariate_selection_variable_names_both, function(x) {
      list(
        Collinearity_selected_covs = unlist(lapply(x, function(y) y[["collinearity_preds"]])),
    ...
    # First count the number of covariates selected under each process for each transition
    Number_of_Covariates_selected_by_Bioregion <- lapply(Bioregion_Covariate_selection_variable_names_both, function(x) {
      lapply(x, function(y) {
        list(
          Num_collinearity_selected_covs = length(y[["collinearity_preds"]]),
    ...
    MOCT_results <- lapply(Number_of_Covariates_selected_by_Bioregion, function(x) {
      list(
        MOCT_collinearity_selection = MOCT(unlist(lapply(x, function(y) y[["Num_collinearity_selected_covs"]]))),
  ...
  if (summary_level == "Initial_lulc") {
    ...
    # First count the number of covariates selected under each process for each transition
    Number_of_Covariates_selected_by_lulc <- lapply(Covariate_selection_variable_names_both_LULC, function(x) {
      lapply(x, function(y) {
        list(
          Num_collinearity_selected_covs = length(y[["collinearity_preds"]]),
          Num_GRRF_preds = length(y[["GRRF_preds"]])
