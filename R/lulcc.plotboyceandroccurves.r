...
lulcc.plotBoyceandROCcurves <- function(RF_specs, GLM_specs, RF_model_num, transition_name) {
  ...
  names(AUC_Boyce_results) <- lapply(names(All_models), function(x) {
    str_replace_all(str_remove(x, paste0("\\.", transition_name)), c(
      unfiltered = "full",
      filtered = "reduced",
      GLM = "LR",
  ...
  ROC_results <- rbindlist(lapply(AUC_Boyce_results, function(x) {
    ROC_df <- data.frame(
      FalsePositive = x[[3]]@x.values, # extract ROCR details to df
  ...
  # Lay out plots
  ROC_boyce_plot <- try(ggarrange(
    plotlist = list(Boyce_plot, ROC_plot),
    labels = c("a.", "b."),
    label.x = 0.00,
    label.y = 0.97,
    ncol = 1,
