...
lulcc.improvetransareadistrib <- function(Scenario_class_area_tables,
  ...
  for (i in 1:reps) {
    ...
    for (scenario in names(old_trans_areas)) {
      ...
      synergistic_trans <- unique(c(
        old_areas_mismatch[[scenario]][["Synergistic_trans"]],
    ...
    for (scenario in names(old_trans_areas)) {
      ...
      undersized_keys <- unique(c(
        names(old_areas_mismatch[[scenario]][["Undersized_target_trans"]]),
        names(old_areas_mismatch[[scenario]][["Undersized_target_trans_0ed"]])
      ))
      Undersized_trans <- setNames(
        mapply(
          c, old_areas_mismatch[[scenario]][["Undersized_target_trans"]][undersized_keys],
      ...
      Oversized_keys <- unique(c(
        names(old_areas_mismatch[[scenario]][["Oversized_target_trans"]]),
        names(old_areas_mismatch[[scenario]][["Oversized_target_trans_0ed"]])
      ))
      Oversized_trans <- setNames(
        mapply(
          c, old_areas_mismatch[[scenario]][["Oversized_target_trans"]][Oversized_keys],
