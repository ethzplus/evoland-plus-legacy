...
## Author: Ben Black
#############################################################################
simulated_map_summaries <- function() {
  ### =========================================================================
  ### - Preparation
  ### =========================================================================
  ...
  pal <- list(
    "Settlement/urban/amenities" = "#BB0011", # Urban
  ...
  for (i in scenario_names[2:4]) {
    ...
    Multi_lyr <- lapp(Scenario_lyrs, fun = function(x, y) {
      x * y
    })
  ...
  Scenario_sankey_alluvial_dfs_start_end <- lapply(scenario_names, function(x) {
    ...
    # extract the scenario paths and load as multi-layer raster
    Scenario_LULC <- rast(Scenario_paths)
    names(Scenario_LULC) <- sapply(Scenario_paths, function(y) {
  ...
  PA_change_results <- lapply(paste(Time_steps), function(x) {
    ...
    if (x == "2060") {
      PA_paths_step <- grep("2055", All_PA_paths, value = TRUE)
    } else {
