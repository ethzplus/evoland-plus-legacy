...
lulc_data_prep <- function(config = get_config(), refresh_cache = FALSE) {
  ...
  # Load JSON of LULC scheme
  scheme <- jsonlite::fromJSON(config[["LULC_aggregation_path"]])

  # Build map of aggregated classes to original numeric codes
  map_numeric <- setNames(
   lapply(scheme, function(cl) {
     sapply(cl$original_classes, `[[`, "value")
    }),
   sapply(scheme, `[[`, "value")
  )
  ...
  rcl <- do.call(rbind, lapply(names(map_numeric), function(k) {
    vals <- map_numeric[[k]]
  ...
  if (is.null(rcl)) stop("The aggregation map contains no source values.")
  rcl <- as.matrix(rcl)

# List input rasters 
tifs <- list.files(config[["rasterized_lulc_dir"]], pattern = "\\.tif(f)?$",
                   full.names = TRUE, ignore.case = TRUE)
if (!length(tifs)) stop("No .tif files found in: ", config[["rasterized_lulc_dir"]])
