...
lulcc.projectedclassareasmismatchstepwise <- function(Scenario_names,
  ...
  Scenario_mismatch_info <- lapply(Scenario_names, function(scenario) {
    ...
    for (n in Sim_years[2:length(Sim_years)]) {
      ...
      Time_step_mistmatches[[n]] <- list(
        class_mismatches = data.frame(class_mismatches),
...
test_stepwise <- lulcc.projectedclassareasmismatchstepwise(
  Scenario_names = names(Mod_trans_area_tables),
  Sim_years = Sim_years,
...
lulcc.improvetransareadistribstepwise <- function(Scenario_class_area_tables,
  ...
  for (i in 1:reps) {
    ...
    for (scenario in names(old_trans_areas)) {
      ...
      for (n in Sim_years[2:length(Sim_years)]) {
        ...
        old_areas_mismatch <- lulcc.projectedclassareasmismatchstepwise(
        ...
        undersized_keys <- unique(c(
          names(old_areas_mismatch[[scenario]][[n]][["Undersized_target_trans"]]),
          names(old_areas_mismatch[[scenario]][[n]][["Undersized_target_trans_0ed"]])
        ))
        Undersized_trans <- setNames(
          mapply(
            c, old_areas_mismatch[[scenario]][[n]][["Undersized_target_trans"]][undersized_keys],
        ...
        Oversized_keys <- unique(c(
          names(old_areas_mismatch[[scenario]][[n]][["Oversized_target_trans"]]),
          names(old_areas_mismatch[[scenario]][[n]][["Oversized_target_trans_0ed"]])
        ))
        Oversized_trans <- setNames(
          mapply(
            c, old_areas_mismatch[[scenario]][[n]][["Oversized_target_trans"]][Oversized_keys],
  ...
  return(list(
    Area_tables = Iteration_area_tables,
...
test_func_stepwise <- lulcc.improvetransareadistribstepwise(
  Scenario_class_area_tables = Mod_area_tables,
...
test_post_syneristic <- lulcc.improvetransareadistrib(
  Scenario_class_area_tables = Mod_area_tables,
...
print(sapply(test_func$Mismatch_checks[[20]], function(x) {
  x[["Total_mismatch_perc"]]
}))
print(sapply(test_post_syneristic$Mismatch_checks[[20]], function(x) {
  x[["Total_mismatch_perc"]]
}))
