...
packs <- c(
  "foreach", "doMC", "data.table", "raster", "tidyverse",
...
compareRaster(Trans_data_stack$Soil_PH, raster_xy_test,
  crs = TRUE, res = FALSE,
...
regional_lulc_dataframes <- lapply(Bioregion_rasters_by_LULC, function(x) {
  ...
  if (nrow(LULC_dataframe) > 0) {
    LULC_dataframe[Transition_potential_cols] <- NA
  }
