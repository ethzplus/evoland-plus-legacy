...
LULC_classes$value <- sapply(LULC_classes$label, function(x) {
  unique(Aggregation_scheme[Aggregation_scheme$Class_abbreviation == x, "Aggregated_ID"])
})
...
lulcc.periodictransmatrices <- function(Raster_combo, Raster_stack, period_name, Step_length) {
  ...
  trans_rates_for_period <- crosstab(
    Raster_stack[[grep(Raster_combo[1], names(Raster_stack))]],
