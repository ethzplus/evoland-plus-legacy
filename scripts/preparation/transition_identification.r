...
LULC_classes$value <- sapply(LULC_classes$label, 
                             function(x) { unique(Aggregation_scheme[Aggregation_scheme$description_edited == x, "ID" ]) })
...
LULC_areal_change$LULC_class <- sapply(LULC_areal_change$value, 
...
names(Calibration_singlestep_tables) <- gsub("Calibration_|_singlestep_trans_table.csv", "", 
...
names(Calibration_multistep_tables) <- gsub("Calibration_|_multistep_trans_table.csv", "", 
