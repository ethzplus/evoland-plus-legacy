...
lulcc.multispectransmodelling <- function(model_specs) {
  ...
  }, future.seed = TRUE, future.packages = packs) # close loop over trnasition datasets
  plan(sequential)
  ...
  if (all(Modelling_check == "Success") == TRUE) {
    ...
    # print error message
    cat(paste0(
      Num_errors, " errors occurred in model fitting and evaluation for the model specification: \n",
