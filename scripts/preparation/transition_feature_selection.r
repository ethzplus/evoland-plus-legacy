...
lapply(list(Pre_PS_folder,
            ...
            PS_results_folder), function(x) dir.create(x, recursive = TRUE, showWarnings = FALSE))

# Predictor table file path is assumed to be defined globally
...
lulcc.featureselection <- function(Dataset_details){
  ...
  Data_paths <- list.files(file.path(Pre_PS_folder, Data_period), pattern = Dataset_scale, full.names = TRUE)
  names(Data_paths) <- basename(Data_paths)
  ...
  saveRDS(Focal_subset, file.path("Data/Preds/Tools/Neighbourhood_details_for_dynamic_updating",
