...
DC_path <- "/app/squashfs-root/usr/bin/DinamicaConsole"
...
if (Pre_check_result == FALSE) { print("Some elements required for modelling are not present/incorrect,
  ...
  #Read in Model.ego file - might have incomplete final line - no warning
  Model_text <- try(readLines("Model/Dinamica_models/LULCC_CH.ego", warn = FALSE))
  #check that Model_text is a non-empty character vector
  if (is.character(Model_text) == FALSE | length(Model_text) == 0) {
      stop("LULCC_CH.ego file not found or empty")
  }

  #Replace dummy string for working directory path
